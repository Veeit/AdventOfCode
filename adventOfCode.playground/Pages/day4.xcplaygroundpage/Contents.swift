...
func part1(parseInput: String) -> [String: Int] {
    ...
    var sleepArray: [String: Int] = [:]
    i = 0
    var sleepCounter = 0
    var startTime = 0
    var sleepMaxTime: [String: Int] = ["0": 0]
    for entry in newArray {
        let id = entry[2]
        let state = entry[3]
        var hasSleep = 0
        if state == "falls" {
            startTime = Int(entry[1].suffix(3).replacingOccurrences(of: "]", with: ""))!
            startTime = startTime - 1
        } else if state == "up" {
            var wakeUpTime = Int(entry[1].suffix(3).replacingOccurrences(of: "]", with: ""))!
            wakeUpTime = wakeUpTime - 1
            sleepCounter = wakeUpTime - startTime
            //            print(sleepCounter) //<-- Stimmt
            if sleepArray[id] != nil {
                hasSleep = sleepArray[id]!
            }
            //            print(sleepCounter)
            
            for entry in sleepMaxTime {
                if entry.value <= sleepCounter {
                    sleepMaxTime.removeAll()
                    sleepMaxTime[id] = sleepCounter
                    print(startTime)
                    print(wakeUpTime)
                    print("==========")
            ...
            sleepArray[id] = sleepCounter + hasSleep
        }
    }
    //    print(sleepArray)
    var maxSleep = 0
    var maxID = ""
    for entry in sleepArray {
        //        print(entry)
...
func part1New(parseInput: String) {
    let inputArray = parseInput.components(separatedBy:  "\n")
    var newArray = inputArray.map{value in value.components(separatedBy:  " ")}
    newArray = newArray.map{value in value.filter{!$0.contains("Guard") && !$0.contains("asleep") && !$0.contains("wakes") && !$0.contains("falles") && !$0.contains("shift")}}
    var correntID = ""
...
//        let time = entry[1]
        var hasSleep = 0
        let date = entry[0].suffix(2)
        newArray[i][0] = String(date)
        
        if state == "falls" {
            startTime = Int(entry[1].suffix(3).replacingOccurrences(of: "]", with: ""))!
            startTime = startTime - 1
            newArray[i][1] = String(startTime)
        } else if state == "up" {
            let wakeUpTime = Int(entry[1].suffix(3).replacingOccurrences(of: "]", with: ""))! - 1
            sleepCounter = wakeUpTime - startTime
            newArray[i-1].insert(String(wakeUpTime), at: 2)
            newArray[i-1].insert(String(sleepCounter), at: 4)
            newArray[i-1].insert(String(id), at: 5)
            newArray.remove(at: i)
...
//        print(sleepMaxTime)
        ...
        newArray[i].remove(at: 3)
        
        i += 1
        
    }
    
    var sleepDic = [String:[AnyObject]]()
    var correntDay = 0
    for entry in newArray {
        i = 0
        let id = entry[4]
        var startTime = Int(entry[1])!
        let doneTime = Int(entry[2])!
        var zeroArray = [Int]()
        let date = String(entry[0])
        
        if sleepDic[id] == nil {
            while zeroArray.count < 60 {
                zeroArray.append(0)
            }
        } else {
            zeroArray = sleepDic[id] as! [Int]
        }
        
        
        while startTime < doneTime {
            if correntDay == 0 {
                correntDay = Int(date)!
//                zeroArray[startTime] = zeroArray[startTime] + 1
            } else if correntDay != Int(date)! {
                correntDay = Int(date)!
                zeroArray[startTime] = zeroArray[startTime] + 1
            } else {
                zeroArray[startTime] = zeroArray[startTime] + 1
...
//            day += 1
//        }
        sleepDic[id] = zeroArray as [AnyObject]
        
    }
    for entry in sleepDic {
        var maxSleepID = sleepMaxTime.first?.key
        var maxSleepCount = 0
        
//        if entry.key == maxSleepID {
////            for sleep in entry.value {
//////                var shit = String(sleep as! String)!
////                maxSleepCount = Int(sleep as! String)!
////                print(sleep)
////            }
//        }
//        print(entry)
        var pos = 0
        if entry.key == maxSleepID {
//            for number in Array(entry.value) {
//                let maxNumber = Int(number as! NSNumber)
//                print(maxNumber)
//                if maxNumber > maxSleepCount {
//                    maxSleepCount = maxNumber
//                }
//            }
            let test: [Int] = entry.value as! [Int]
            i = 1
            
            for number in test {
                if number > maxSleepCount {
                    maxSleepCount = number
//                    print(i)
                    pos = i
                }
                i += 1
...
//print(part1(parseInput: test1))
print(part1New(parseInput: test1))
